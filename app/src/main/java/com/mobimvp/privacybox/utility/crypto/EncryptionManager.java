...
public class EncryptionManager {
     ...
     *
     * @param password 密码
     * @param secKey   由initEncryptionManager生成的安全密钥（可以从文件中读取）
     * @throws Exception
     */
     ...
     *
     * @param password 密码
     * @param secKey   initEncryptionManager生成的内部密钥
     * @throws Exception 如果校验失败，则会抛出异常
     */
     ...
     *
     * @param in       需要加密的原始文件（文件将会被直接加密）
     * @param listener 加密回调函数（可选）
     ...
     *
     * @param in       需要解密的原始文件
     * @param out      解密后的文件（如果文件是快速加密，则此参数不会被使用）
     * @param listener 加密回调函数（可选），如果置为null，则您将不会获知此文件为快速加密，还是完整加密
     * @throws Exception 加密失败
     */
    ...
    public class Encryptor {
         ...
         * 加密数据
         *
         * @param rawData 需要加密的原始数据
         * @param isFinal 当传送最后一组数据时为真，当随后需要继续加密数据时，为假
         * @return 加密后的数据
         * @throws Exception 加密出现错误
         */
